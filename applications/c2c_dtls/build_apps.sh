...
#  generates routing information macros
...
for p in "${PROFILES[@]}"; do
    ...
    sed -i '/CONFIG_LWM2M_CLIENT_URI/d' ./$config.config
    echo "CONFIG_LWM2M_CLIENT_URI=\"coap://[${NODES[$other_node]}]\"" >> ./$config.config
done
...
for p in "${PROFILES[@]}"; do
    ...
    then
        ...
        sed -i '/CONFIG_LWM2M_CLIENT_URI/d' $config_file
        echo "CONFIG_LWM2M_CLIENT_URI=\"coap://[${GLOBAL_ADDRS[$other_node]}]\"" >> $config_file
    fi
